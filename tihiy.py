...
def generate_inline_keyboard():
    ...
    # Добавляем первую кнопку "Условия аренды" с синим цветом
    keyboard.add_button('Условия аренды', color=VkKeyboardColor.PRIMARY)

    # Добавляем вторую кнопку "Описание и фото" с зеленым цветом
    keyboard.add_button('Описание и фото', color=VkKeyboardColor.POSITIVE)

    # Добавляем третью кнопку "Связь с админом" с красным цветом
...
'\nДоставка обсуждается индивидуально🚚', keyboard=keyboard)
                ...
                keyboard = generate_inline_keyboard()  # Генерируем inline клавиатуру для сообщения с условиями аренды
...
'\n6. Мы встречаемся и обмениваем залог на наш комплект.', keyboard=keyboard)
